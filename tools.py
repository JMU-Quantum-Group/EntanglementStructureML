...
def compute_all_stretchable(P, N):
    ...
    for k in range(1 - N, N - 1):
        ...
        result_item = compute(P, generate_k_stretchable_partitions(N, k))
...
def compute_all_producible(P, N):
    result_list = list()
    for k in range(1, N):
        print(str(N) + "partite" + str(k) + "-prod:")
        result_item = compute(P, generate_k_producible_partitions(N, k))
        result_list.append(result_item)
    return result_list
...
def compute_all_partitionable(P, N):
    ...
    for k in range(N - 1, 1, -1):
        ...
        result_item = compute(P, generate_k_partitionable_partitions(N, k))
        result_list.append(result_item)
    return result_list
